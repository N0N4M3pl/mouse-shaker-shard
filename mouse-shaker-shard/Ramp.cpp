...
void _ramp<T>::init(T _val) {
  ...
  pos = 0;
  // grain = 10;
  // loop = ONCEFORWARD; //FORTHANDBACK; //ONCEFORWARD;
  // speed = FORWARD;
  // paused = false;
  // automated = true;
}
...
// template<class T>
// T _ramp<T>::go(T _val, unsigned int _dur) {
//   return go(_val, _dur, LINEAR);
// }

// template<class T>
// T _ramp<T>::go(T _val, unsigned int _dur, ramp_mode _mode) {
//   return go(_val, _dur, _mode, FORTHANDBACK);
// }

template<class T>
T _ramp<T>::go(T _val, unsigned int _dur, ramp_mode _mode /*, loop_mode _loop*/) {
  A = val;
  B = _val;
  mode = _mode;
  dur = _dur;
...
// template<class T>
// void _ramp<T>::setGrain(unsigned int _grain) {
//   grain = _grain;
// }
